            ...
            <div class="w-full">
              <div class="mb-4 text-right">
                <Link
                  ...
                  :href="route('conocimientos.create')"
                >
              ...
              <table class="w-full">
                <thead class="font-bold bg-gray-300 border-b-2">
                  ...
                  <td class="px-4 py-2">Ult. Act.</td>
                  <td class="px-4 py-2">Acción</td>
                </thead>
                <tbody>
                  <tr v-for="conocimiento in conocimientos.data" :key="conocimiento.id">
                    <td class="px-4 py-2">{{ conocimiento.id }}</td>
                    <td class="px-4 py-2">
                      <Link
                        class="text-blue-700"
                        :href="route('conocimientos.show', conocimiento.id)"
                        >{{ conocimiento.descripcion }}</Link
                      >
                    </td>
                    <td class="px-4 py-2">{{ conocimiento.tipo.nombre }}</td>
                    <td class="px-4 py-2">{{ formatDate(conocimiento.updated_at) }}</td>
...
const { conocimientos, filters } = toRefs(props)
...
watch(params, throttle(() => {
  Inertia.get(route('conocimientos.index'), pickBy({ ...params }, (value) => value), {
    replace: true,
    preserveState: true,
    preserveScroll: true
  })
