<template>
  ...
  <breeze-authenticated-layout>
          ...
          >
            <div class="w-full">
              ...
              <table class="w-full">
                ...
                <tbody>
                  <tr v-for="medio in medios.data" :key="medio.id">
                    <td class="px-4 py-2">{{ medio.id }}</td>
                    <td class="px-4 py-2">
                      <Link
                        class="text-blue-700"
                    ...
                    <td class="px-4 py-2">
                      <span class="block text-ellipsis w-24 overflow-hidden whitespace-nowrap"
                        :title="medio.bio">{{ medio.bio }}</span>
                    </td>
    ...
    <modal-confirm
      ...
      confirm-message="¿Esta seguro de borrar a el medio?"
...
const { medios, filters } = toRefs(props)
...
watch(params, throttle(() => {
  Inertia.get(route('medios.index'), pickBy({ ...params }, (value) => value), {
    replace: true,
    preserveState: true,
    preserveScroll: true
  })
