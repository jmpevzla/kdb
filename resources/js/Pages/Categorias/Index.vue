                  ...
                  <tr v-for="categoria in categorias.data" :key="categoria.id">
                    ...
                    <td class="px-4 py-2">
                      <span class="block text-ellipsis w-24 overflow-hidden whitespace-nowrap"
                        :title="categoria.descripcion">{{ categoria.descripcion }}</span>
                    </td>
...
const { categorias, filters } = toRefs(props)
...
watch(params, throttle(() => {
  Inertia.get(route('categorias.index'), pickBy({ ...params }, (value) => value), {
    replace: true,
    preserveState: true,
    preserveScroll: true
  })
